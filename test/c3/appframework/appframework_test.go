...
var _ = Describe("c3appfw test", func() {
	...
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) and App Framework", func() {
		It("integration, c3, appframeworkc3, appframework: can deploy a C3 SVA with App Framework enabled, install an app, then disable it by using a disabled version of the app and then remove it from app source", func() {
	...
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) and App Framework", func() {
		It("integration, c3, appframeworkc3, appframework: can deploy a C3 SVA with App Framework enabled and update apps after app download is completed", func() {
	...
	Context("Single Site Indexer Cluster with Search Head Cluster (C3) and App Framework", func() {
		It("smoke, c3, appframeworkc3, appframework: can deploy a C3 SVA with App Framework enabled and check isDeploymentInProgressFlag for CM and SHC CR's", func() {
	...
	Context("Clustered deployment (C3 - clustered indexer, search head cluster)", func() {
		It("integration, c3: can deploy a C3 SVA and a Standalone, then add that Standalone as a Search Head to the cluster", func() {
			...
			*/
			...
			Expect(err).To(Succeed(), "Unable to deploy Single Site Indexer Cluste")
			...
			lastInd := strings.LastIndex(deployment.GetName(), "-")
			ns := deployment.GetName()[:lastInd]
			standalonePodName := fmt.Sprintf(testenv.StandalonePod, deployment.GetName(), 0)
			confFilePath := "/opt/splunk/etc/system/local/server.conf"
			config := "master_uri"
			cmPodName := fmt.Sprintf(testenv.ClusterManagerPod, deployment.GetName())
			lastInd = strings.LastIndex(cmPodName, "-")
			cmService := cmPodName[:lastInd]
			value := "master_uri = https://" + cmService + "-service:8089"

			testenv.VerifyConfOnPod(deployment, ns, standalonePodName, confFilePath, config, value)
