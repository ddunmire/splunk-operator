	...
	AfterEach(func() {
		...
		if filePresentOnOperator {
			...
			for index := 1; index <= 15; index++ {
				podDownloadPath := filepath.Join(testenv.AppDownloadVolume, fmt.Sprintf("test_file_%d.img", index))
	...
	Context("Standalone deployment (S1) with App Framework", func() {
		It("integration, s1, appframeworks1, appframework: can deploy a Standalone instance and install a bigger volume of apps than the operator PV disk space", func() {
			...
			*/
			...
			opPod := testenv.GetOperatorPodName(testcaseEnvInst)
			for index := 1; index <= 15; index++ {
				err := testenv.CreateDummyFileOnOperator(ctx, deployment, opPod, testenv.AppDownloadVolume, "100MB", fmt.Sprintf("test_file_%d.img", index))
